...
@st.cache_resource
def load_summarizer():
    return pipeline("summarization", model="facebook/bart-large-cnn", device=-1)  # -1 = CPU
...
if st.button("Summarize"):
    if text.strip():
        with st.spinner("Summarizing..."):
            ...
            st.markdown(f"<div class='glassmorphism-card'><h3>Summary</h3><p>{summary[0]['summary_text']}</p></div>", unsafe_allow_html=True)
    else:
